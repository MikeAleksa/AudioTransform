...
def augment(input_dir: Path, output_dir: Path):
    ...
    while len(filelist) > 1:
        ...
        #   varispeed between [0.95, 1.05]
        reduction = [0, -3, -6, -9, -12, -15]
        f1 = AudioFile(path=f1).varispeed(uniform(0.95, 1.05)).gain(choice(reduction))
        f2 = AudioFile(path=f2).varispeed(uniform(0.95, 1.05)).gain(choice(reduction))
